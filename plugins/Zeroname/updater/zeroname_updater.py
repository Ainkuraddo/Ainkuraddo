...
def processNameOp(domain, value, test=False):
    ...
    try:
        ...
        return False
    if "ainkuraddo" not in data and "map" not in data:
    # Namecoin standard use {"map": { "blog": {"ainkuraddo": "1D..."} }}
        print("No ainkuraddo and no map in ", data.keys())
        return False
    if "map" in data:
    # If subdomains using the Namecoin standard is present, just re-write in the Ainkuraddo way
    # and call the function again
        data_map = data["map"]
        new_value = {}
        for subdomain in data_map:
            if "ainkuraddo" in data_map[subdomain]:
                new_value[subdomain] = data_map[subdomain]["ainkuraddo"]
        if "ainkuraddo" in data and isinstance(data["ainkuraddo"], string_types):
        # {
        #    "ainkuraddo":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9",
        #    ....
        # }
            new_value[""] = data["ainkuraddo"]
        if len(new_value) > 0:
            return processNameOp(domain, json.dumps({"ainkuraddo": new_value}), test)
        else:
            return False
    if "ainkuraddo" in data and isinstance(data["ainkuraddo"], string_types):
    # {
    #    "ainkuraddo":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9"
    # } is valid
        return processNameOp(domain, json.dumps({"ainkuraddo": { "": data["ainkuraddo"]}}), test)
    if not isinstance(data["ainkuraddo"], dict):
        print("Not dict: ", data["ainkuraddo"])
        return False
    ...
    for subdomain, address in data["ainkuraddo"].items():
...
if not os.path.isfile(config_path):  # Create sample config
    open(config_path, "w").write(
        json.dumps({'site': 'site', 'ainkuraddo_path': '/home/ainkuraddo', 'privatekey': '', 'lastprocessed': 223910}, indent=2)
    )
    print("* Example config written to %s" % config_path)
    sys.exit(0)

config = json.load(open(config_path))
names_path = "%s/data/%s/data/names.json" % (config["ainkuraddo_path"], config["site"])
os.chdir(config["ainkuraddo_path"])  # Change working dir - tells script where Ainkuraddo install is.

# Parameters to sign and publish
command_sign_publish = [sys.executable, "ainkuraddo.py", "siteSign", config["site"], config["privatekey"], "--publish"]
if sys.platform == 'win32':
...
assert processBlock(223911, test=True) # Testing ainkuraddowork.bit
