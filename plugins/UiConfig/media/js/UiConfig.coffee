...
class UiConfig extends ZeroFrame
	...
	onOpenWebsocket: =>
		@cmd("wrapperSetTitle", "Config - Ainkuraddo")
	...
	renderBottomRestart: =>
		...
		h("div.bottom.bottom-restart", {classes: {visible: values_pending.length and not values_changed.length}}, h("div.bottom-content", [
			...
			h("a.button.button-submit.button-restart", {href: "#Restart", classes: {loading: @restart_loading}, onclick: @handleRestartClick}, "Restart Ainkuraddo client")
		]))
