...
class SiteManagerPlugin(object):
    def load(self, *args, **kwargs):
        ...
        self.log = logging.getLogger("AinkuraddoLocal Plugin")
        ...
        if not config.namecoin_host or not config.namecoin_rpcport or not config.namecoin_rpcuser or not config.namecoin_rpcpassword:
            ...
            self.log.error("Missing parameters to connect to namecoin node. Please check all the arguments needed with '--help'. Ainkuraddo will continue working without it.")
            return
        ...
        try:
            ...
            self.log.error("The Namecoin node is unreachable. Please check the configuration value are correct. Ainkuraddo will continue working without it.")
    ...
    def resolveDomain(self, domain):
        ...
        if "ainkuraddo" in domain_object["value"]:
            ainkuraddo_domains = json.loads(domain_object["value"])["ainkuraddo"]

            if isinstance(ainkuraddo_domains, str):
                # {
                #    "ainkuraddo":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9"
                # } is valid
                ainkuraddo_domains = {"": ainkuraddo_domains}

            self.cache[domain] = {"addresses_resolved": ainkuraddo_domains, "time": time.time()}

        elif "map" in domain_object["value"]:
            # Namecoin standard use {"map": { "blog": {"ainkuraddo": "1D..."} }}
            data_map = json.loads(domain_object["value"])["map"]

            ainkuraddo_domains = dict()
            for subdomain in data_map:
                if "ainkuraddo" in data_map[subdomain]:
                    ainkuraddo_domains[subdomain] = data_map[subdomain]["ainkuraddo"]
            if "ainkuraddo" in data_map and isinstance(data_map["ainkuraddo"], str):
                # {"map":{
                #    "ainkuraddo":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9",
                # }}
                ainkuraddo_domains[""] = data_map["ainkuraddo"]

            self.cache[domain] = {"addresses_resolved": ainkuraddo_domains, "time": time.time()}

        else:
            # No Ainkuraddo address registered
            return None
