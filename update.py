...
def download():
    ...
    urls = [
        "https://github.com/Ainkuraddo/Ainkuraddo/archive/master.zip",
        "https://gitlab.com/Ainkuraddo/Ainkuraddo/repository/archive.zip?ref=master",
        "https://try.gogs.io/Ainkuraddo/Ainkuraddo/archive/master.zip"
    ]
...
def update():
    ...
    if os.path.isfile(updatesite_path + "/content.json") and len(updatesite_bad_files) == 0 and sites_json.get(config.updatesite, {}).get("serving"):
        ...
        # Keep file only in Ainkuraddo directory
        inner_paths = [inner_path for inner_path in inner_paths if inner_path.startswith("Ainkuraddo/")]
    else:
    ...
    for inner_path in inner_paths:
        ...
        print ".",
        dest_path = re.sub("^([^/]*-master.*?|Ainkuraddo)/", "", inner_path)  # Skip root ainkuraddo-master-... like directories
