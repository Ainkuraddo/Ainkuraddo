...
class UiRequest(object):
    ...
    def route(self, path):
        ...
        # Internal functions
        elif "/Ainkuraddo-Internal/" in path:
            path = re.sub(".*?/Ainkuraddo-Internal/", "/", path)
    ...
    def actionWrapper(self, path, extra_headers=None):
        ...
        if match:
            ...
            # Make response be sent at once (see https://github.com/Ainkuraddo/Ainkuraddo/issues/1092)
    ...
    def processQueryString(self, site, query_string):
        match = re.search("ainkuraddo_peers=(.*?)(&|$)", query_string)
        if match:
