...
class Config(object):
    ...
    def createArguments(self):
        ...
        config_file = self.start_dir + "/ainkuraddo.conf"
        ...
        self.parser.add_argument('--version', action='version', version='Ainkuraddo %s r%s' % (self.version, self.rev))
    ...
    def loadTrackersFile(self):
        ...
        try:
            ...
            else:  # Relative to ainkuraddo.py
    ...
    def parse(self, silent=False, parse_config=True):
        ...
        if silent:  # Restore original functions
            if self.parser.exited and self.action == "main":  # Argument parsing halted, don't start Ainkuraddo with main action
                self.action = None
