...
class TestContent:
    ...
    def testSign(self, site, inner_path):
        ...
        assert (
            {key: val for key, val in content_old.items() if key not in ["modified", "signs", "sign", "ainkuraddo_version"]}
            ==
            {key: val for key, val in content.items() if key not in ["modified", "signs", "sign", "ainkuraddo_version"]}
        )
